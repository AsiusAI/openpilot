...
Commit the generated model through Git LFS. This fork routes LFS to the Asius GitHub repository so the QNN model and
the inherited openpilot models are available to CI and device checkouts without requiring write access to comma's store.
